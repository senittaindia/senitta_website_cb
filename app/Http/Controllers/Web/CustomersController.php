...
class CustomersController extends DataController
{
    ...
    public function handleSocialLoginCallback($social){
		...
		if($social=="google" && $_POST){
    		...
    		$customers_firstname = substr($user->name, 0, strpos($user->name, ' '));
    		$customers_lastname = str_replace($customers_firstname.' ', '', $user->name);
		...
		if($validator->fails()){
			...
			if(count($user_email)>0){
				return redirect('/signup')->withInput($request->input())->with('error', Lang::get("website.Email already exist"));
			 	...
			 	$customers_id=	DB::table('customers')->insertGetId($data);
		//	exit();
					//check authentication of email and password
				
					//$customer2 = auth()->guard('customer')->user();
										$hygiene_currency_data_array = array(
                                                        "customers_id"=>$customers_id,
                                                        "created_at"=>time(),
                                                        "balance_amount"=>0.00
