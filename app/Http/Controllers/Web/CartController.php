...
//		}
	...
	public function addToCart(Request $request){
		...
		$product_stock = DB::table('inventory')->where('products_id', $request->products_id);
		$product_in_stock = $product_stock->where('stock_type','in')->sum('stock');
		$product_stocks = DB::table('inventory')->where('products_id', $request->products_id);
		$product_out_stock = $product_stocks->where('stock_type','out')->sum('stock');
		$total_stock = $product_in_stock -  $product_out_stock;
						...
						if($coupons->minimum_amount > 0 and $coupons->minimum_amount >= $cart_price){							
							...
							$response = array('success'=>'2', 'message'=>Lang::get("website.Coupon amount limit is exceeded than maximum price"));
								...
								if($coupons->discount_type=='fixed_cart'){
									...
									elseif($coupons->discount_type=='hygiene_cashback'){
