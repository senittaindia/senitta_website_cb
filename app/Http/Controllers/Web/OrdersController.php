...
class OrdersController extends DataController
{
	...
	public function checkout(Request $request){
		...
		$hygeine_amount = DB::table('customers_hygiene_currency')
			                  ->where('customers_id',session('customers_id'))
                                ->first();	
                                
        $result['hygeine_balance_amount'] = $hygeine_amount->balance_amount;
            //   print_r($hygeine_amount->balance_amount);die();
	...
	public function checkout_payment_method(Request $request){
		...
		foreach($request->all() as $key=>$value){
		  ...
		  if($key=='shipping_price' and !empty($result['commonContent']['setting'][82]->value) and $result['commonContent']['setting'][82]->value <= session('sub_price')){
	...
	//order_detail
	public function paymentComponent(Request $request){		//dd('hi');
	...
	public function place_order(Request $request){		
		...
		$hygine_currency           				=   session('hygine_currency');
...
// 		}
    ...
    $shipping_charges = DB::table('flate_rate')->first();
        ...
        if(!empty(session('coupon_shipping_free')) and session('coupon_shipping_free')=='yes'){
        			...
        			//price
        			if(!empty(session('shipping_detail'))){
        			    if(session('products_price') > 500){
        			        $shipping_cost = 0;
        			    } else {
        				    $shipping_cost = $shipping_charges->flate_rate;;
        			    }
        			}else{
        				$shipping_cost = 0;
        			}
		...
		$order_price = (session('products_price')+$tax_rate+$shipping_cost)-($hygine_currency+$coupon_discount);	
...
//						'updatedAt'=>$result->transaction->updatedAt->date,
			...
			$ccavenue = DB::table('payment_description')->where([['payment_name','ccavenue'],['language_id',Session::get('language_id')]])->get();
			$payment_method = $ccavenue[0]->name;
			$payment_status=$response['order_status'];
				$orderweb_id=$response['order_id'];
				$reference=$response['tracking_id'];
		} 
		...
		// 			  'hygiene_currency'  =>    $hygine_currency,
		// 			 'shipping_cost' =>$shipping_cost,
		// 			 'shipping_method'  =>  $shipping_method,
		// 			 'order_id' => $orderweb_id,
		...
		// 		]);
		// 		//Payment history
		// 	 $payment_history_id = DB::table('payment')->insertGetId(
		...
		// 	return redirect('orders')->with('success', Lang::get("website.Payment has been processed successfully"));
			...
			$on_purchase = DB::table('settings')->where('name', 'senitta_hygiene_currency_on_purchase')->first();
				$payback_hygiene_currency = ( $order_price * $on_purchase->value ) / 100;
			$orders_id = DB::table('orders')->insertGetId(
				[	 'customers_id' => $customers_id,
					 'customers_name'  => $delivery_firstname.' '.$delivery_lastname,
					 'customers_street_address' => $delivery_street_address,
			...
			if(!empty(session('coupon'))){
					...
					$hygiene_currency_data5 = DB::table('customers_hygiene_currency')
				->select('customers_hygiene_currency_id','balance_amount')
			...
			foreach(session('coupon') as $coupons_data){
				...
				//update coupans		
			//	$coupon_id = DB::statement("UPDATE `coupons` SET `used_by`= CONCAT(used_by,',$customers_id') WHERE `code` = '".$coupons_data->code."'");
				$code4=$coupons_data->code;
					
			}
			$Totaladd=$hygiene_currency_data5->balance_amount +$coupon_amount;
				DB::table('customers_hygiene_currency')->where('customers_hygiene_currency_id', $hygiene_currency_data5->customers_hygiene_currency_id)->update(['balance_amount' => $Totaladd]);
				//insert hygiene currency data into customers_hygiene_currency_passbook
				$customers_hygiene_currency_passbook_data5 = array(
					'customers_hygiene_currency_id' 	=>  $hygiene_currency_data5->customers_hygiene_currency_id,
					'transaction_type'					=>  'credit',
					'coupon'=>json_encode(['order_id'=>$orders_id,'code'=>$code4]),
					'transaction_amount'				=>  $coupon_amount,
					'transaction_message' 				=>  'congratulation! '.$coupon_amount.' hygiene currency added to your account.',
					'created_at' 						=>  time()
				);
				$customers_hygiene_currency_passbook_id5 = DB::table('customers_hygiene_currency_passbook')->insertGetId($customers_hygiene_currency_passbook_data5);
			
			
		}
				
				
					$hygiene_currency_data = DB::table('customers_hygiene_currency')
				->select('customers_hygiene_currency_id','balance_amount')
				->where('customers_id', '=', $customers_id)
				->first();
                // Log::error('GET hygiene_currency_data');
                // Log::error(json_encode($hygiene_currency_data));
                
                
                
                
                
                if(empty($hygiene_currency_data)){
                    $hygiene_currency_data_array = array(
                                                        "customers_id"=>$customers_id,
                                                        "created_at"=>time(),
                                                        "balance_amount"=>0.00
                                                    );
                    DB::table('customers_hygiene_currency')->insert($hygiene_currency_data_array);
                    $hygiene_currency_data = DB::table('customers_hygiene_currency')
                            				->select('customers_hygiene_currency_id','balance_amount')
                            				->where('customers_id', '=', $request->customers_id)
                            				->first();
                    // Log::error('GET hygiene_currency_data after insert');
                    // Log::error(json_encode($hygiene_currency_data));
                }
                	if($hygiene_currency_data->balance_amount > $hygine_currency){
					$leftHygieneCurrency = $hygiene_currency_data->balance_amount - $hygine_currency;
					
					//update hygiene currency account on referral
					DB::table('customers_hygiene_currency')->where('customers_hygiene_currency_id', $hygiene_currency_data->customers_hygiene_currency_id)->update(['balance_amount' => $leftHygieneCurrency]);

					//insert hygiene currency data into customers_hygiene_currency_passbook
					$customers_hygiene_currency_passbook_data = array(
						'customers_hygiene_currency_id' 	=>  $hygiene_currency_data->customers_hygiene_currency_id,
						'transaction_type'					=>  'debit',
						'transaction_amount'				=>  $hygine_currency,
						'transaction_message' 				=>  $hygine_currency.' hygiene currency debited from your account.',
						'created_at' 						=>  time()
					);
					$customers_hygiene_currency_passbook_id = DB::table('customers_hygiene_currency_passbook')->insertGetId($customers_hygiene_currency_passbook_data);
					
					
                	}
                	$hygiene_currency_data1 = DB::table('customers_hygiene_currency')
				->select('customers_hygiene_currency_id','balance_amount')
				->where('customers_id', '=', $customers_id)
				->first();
				$total_hygiene_currency = $payback_hygiene_currency + $hygiene_currency_data1->balance_amount;
				//update hygiene currency account on referral
				DB::table('customers_hygiene_currency')->where('customers_hygiene_currency_id', $hygiene_currency_data1->customers_hygiene_currency_id)->update(['balance_amount' => $total_hygiene_currency]);

				//insert hygiene currency data into customers_hygiene_currency_passbook
				$customers_hygiene_currency_passbook_data1 = array(
					'customers_hygiene_currency_id' 	=>  $hygiene_currency_data1->customers_hygiene_currency_id,
					'transaction_type'					=>  'credit',
					'transaction_amount'				=>  $payback_hygiene_currency,
					'transaction_message' 				=>  'congratulation! '.$payback_hygiene_currency.' hygiene currency added to your account.',
					'created_at' 						=>  time()
				);
				$customers_hygiene_currency_passbook_id1 = DB::table('customers_hygiene_currency_passbook')->insertGetId($customers_hygiene_currency_passbook_data1);
			 $myVar = new CartController();
			 $cart = $myVar->myCart(array());
		...
		foreach($order as $data){
			...
			return redirect('checkout')->with('error', Lang::get("website.Error while placing order"));		
		}
		else if($payment_status == "Failure"){
		   
			return redirect('checkout')->with('error', Lang::get("website.Error while placing order"));		
		}
		else if($payment_status == "Invalid"){
			return redirect('checkout')->with('error', Lang::get("website.Error while placing order"));		
		}
	...
	public function getPaymentMethods(){
		...
		$ccavenue_description = DB::table('payment_description')->where([['payment_name','ccavenue'],['language_id',Session::get('language_id')]])->get();
		$ccavenue = array(
			'environment' => 'Live', 
			'name' => $ccavenue_description[0]->name, 
			'public_key' => $payments_setting[0]->ccavenue_merchant_id,
			'access_code' => $payments_setting[0]->ccavenue_access_code,
			'working_key' => $payments_setting[0]->ccavenue_working_key,
			'active' => $payments_setting[0]->ccavenue_active,
			'payment_currency' => $payments_setting[0]->payment_currency,
	...
	public function updatestatus(Request $request){
		if(!empty($request->orders_id)){
			...
			$hygiene_currency_data = DB::table('orders')
                ->select('customers_hygiene_currency_passbook.*')
				->where('orders_id', '=', $request->orders_id)
				->join('customers','customers.customers_id','=','orders.customers_id')
				->join('customers_hygiene_currency','customers.customers_id','=','customers_hygiene_currency.customers_id')
				->join('customers_hygiene_currency_passbook','customers_hygiene_currency_passbook.customers_hygiene_currency_id','=','customers_hygiene_currency.customers_hygiene_currency_id')
				->where('customers_hygiene_currency_passbook.coupon', '!=', '0')
				->where('customers_hygiene_currency_passbook.coupon', 'LIKE', '%order_id":'.$request->orders_id.'%')
				->first();
			if($hygiene_currency_data){
			    $customers_hygiene_currency = DB::table('customers_hygiene_currency')
                            			        ->where('customers_hygiene_currency_id','=',$hygiene_currency_data->customers_hygiene_currency_id)
                            			        ->first();
                DB::table('customers_hygiene_currency')
                            			        ->where('customers_hygiene_currency_id','=',$hygiene_currency_data->customers_hygiene_currency_id)
                            			        ->update(['balance_amount'=>$customers_hygiene_currency->balance_amount-$hygiene_currency_data->transaction_amount]);
                            			        
                DB::table('customers_hygiene_currency_passbook')->where('id','=',$hygiene_currency_data->id)->delete();            			        
			}
