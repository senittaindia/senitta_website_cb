...
class ProductsController extends DataController
{
     ...
     */
		public function periodtracker(Request $request){
		  $title = array('pageTitle' => 'Period Tracker');
		$result = array();			
		$result['commonContent'] = $this->commonContent();
		
		return view("periodtracker", $title)->with('result', $result);   
		}
	//shop 
	...
	public function productDetail(Request $request){
		...
		$reviews = array();
		$reviews['data'] = DB::table('reviews')
		                            ->select('reviews.reviews_rating','reviews.review_msg','reviews.date_added','customers.customers_firstname','customers.customers_lastname','customers.customers_picture')
		                            ->join('customers', 'customers.customers_id', '=', 'reviews.customers_id')
		                            ->where('reviews.products_id','=', $products[0]->products_id)
		                            ->orderBy('reviews.reviews_id', 'desc')
		                            ->limit(5)
		                            ->get();
        $reviews_first_data = DB::table('reviews')
		                            ->join('customers', 'customers.customers_id', '=', 'reviews.customers_id')
		                            ->where('reviews.products_id','=', $products[0]->products_id)
		                            ->orderBy('reviews.reviews_id', 'desc')
		                            ->limit(5)
		                            ->first();
		                          //  print_r($reviews_first_data);die();

		$reviews['data1'] = DB::table('reviews')
		                            ->select('reviews.reviews_rating','reviews.review_msg','reviews.date_added','customers.customers_firstname','customers.customers_lastname','customers.customers_picture')
		                            ->leftjoin('customers', 'customers.customers_id', '=', 'reviews.customers_id')
		                            ->where([['reviews.products_id','=', $products[0]->products_id],['reviews.reviews_id','!=',$reviews_first_data->reviews_id]])
		                            ->orderBy('reviews.reviews_id', 'desc')
		                            ->get();                            
		$reviews['avg_rate'] = DB::table('reviews')
		                            ->where('reviews.products_id','=', $products[0]->products_id)
		                            ->avg('reviews_rating');
		$reviews['total_ratings'] = DB::table('reviews')
		                            ->where('reviews.products_id','=', $products[0]->products_id)
		                            ->count();
		for($rIndex=1;$rIndex<=5;$rIndex++){
		    $reviews['counter'][] = array(
		                                ...
		                                'count'=>DB::table('reviews')
        				                            ->where('reviews.products_id','=', $products[0]->products_id)
        				                            ->where('reviews.reviews_rating','=', $rIndex)
        				                            ->count()
		                            );
		}
