...
class AdminPaymentController extends Controller
{
	...
	public function paymentsetting(Request $request){
		...
		foreach($result['languages'] as $languages_data){
			...
			$ccavenue = DB::table('payment_description')->where([
				['language_id', '=', $languages_data->languages_id],
				['payment_name', '=', $shipping_methods[0]->ccavenue_name],
			])->get();
			
    		if(count($ccavenue)>0){								
    			$ccavenue_description[$languages_data->languages_id]['name'] = $ccavenue[0]->name;
    			$ccavenue_description[$languages_data->languages_id]['language_name'] = $languages_data->name;
    			$ccavenue_description[$languages_data->languages_id]['languages_id'] = $languages_data->languages_id;										
    		}else{
    			$ccavenue_description[$languages_data->languages_id]['name'] = '';
    			$ccavenue_description[$languages_data->languages_id]['language_name'] = $languages_data->name;
    			$ccavenue_description[$languages_data->languages_id]['languages_id'] = $languages_data->languages_id;	
    		}
		...
		$result['ccavenue_description'] 		 = $ccavenue_description;
		...
		$result['shipping_methods']	= $shipping_methods;//dd($result);
	...
	public function updatePaymentSetting(Request $request){
		...
		DB::table('payments_setting')->where('payments_id', '=', '1')->update([
				...
				'ccavenue_active' 		=>   $request->ccavenue_active,
		...
		$ccavenue_name = $request->ccavenue_name;
		...
		// CCAvenue 
			...
			$ccavenueName = 'ccavenue_name_'.$languages_data->languages_id;
			
			$checkExist = DB::table('payment_description')->where('payment_name','=',$ccavenue_name)->where('language_id','=',$languages_data->languages_id)->get();
			
			
			if(count($checkExist)>0){
				DB::table('payment_description')->where('payment_name','=',$ccavenue_name)->where('language_id','=',$languages_data->languages_id)->update([
					'name'  	    		 =>   $request->$ccavenueName,
					]);
			}else{
